...
CREATE TABLE forum_categories (
    ...
    parent_id TEXT REFERENCES forum_categories(id) ON DELETE CASCADE,
    slug TEXT NOT NULL,
    name TEXT NOT NULL,
    description TEXT,
    color TEXT,
...
CREATE TABLE forum_topics (
    ...
    last_post_at TEXT,
    last_post_by TEXT REFERENCES users(id) ON DELETE SET NULL,
    created_at TEXT DEFAULT (datetime('now')),
    updated_at TEXT DEFAULT (datetime('now')),
    UNIQUE(site_id, slug)
);
...
CREATE TABLE forum_mentions (
    ...
    site_id TEXT REFERENCES sites(id) ON DELETE CASCADE,
    user_id TEXT REFERENCES users(id) ON DELETE CASCADE,
    mentioned_by TEXT REFERENCES users(id) ON DELETE CASCADE,
    topic_id TEXT REFERENCES forum_topics(id) ON DELETE CASCADE,
    post_id TEXT REFERENCES forum_posts(id) ON DELETE CASCADE,
...
CREATE TABLE forum_user_badges (
    ...
    site_id TEXT REFERENCES sites(id) ON DELETE CASCADE,
    user_id TEXT REFERENCES users(id) ON DELETE CASCADE,
    badge_id TEXT REFERENCES forum_badges(id) ON DELETE CASCADE,
...
CREATE TABLE forum_reputation_log (
    ...
    topic_id TEXT REFERENCES forum_topics(id) ON DELETE SET NULL,
    post_id TEXT REFERENCES forum_posts(id) ON DELETE SET NULL,
    badge_id TEXT REFERENCES forum_badges(id) ON DELETE SET NULL,
    note TEXT,
    created_at TEXT DEFAULT (datetime('now'))
);
